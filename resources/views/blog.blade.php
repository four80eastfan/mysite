...
try:
    ...
    sys.exit(1)
			...
			<div class="content-block">
				...
				<p>Below is my solution to a problem posed on the site Hackerrank: given a graph of weighted undirected edges, write an algorithm that finds a minimum spanning tree. That is, find 'a subset of the edges that forms a tree that includes every node, where the total weight of all the edges in the tree is minimized' (Kruskal's Algorithm - Wikipedia). A complete outline of the problem can be found here: <a href='https://www.hackerrank.com/challenges/kruskalmstrsub'>Kruskal on Hackerrank</a>. The approach to solving this problem is as follows: after ordering all of the edges from the original graph from least to greatest weight, we create a new graph by looking at each edge in order and adding the edge if it doesn't create a cycle or rejecting it if it does. The end result is a minimum spanning tree. The basic idea is to construct a tree using the edges from the original graph as our pieces, until all of the nodes are connected. As we want this tree to be of minimum weight, we start by ordering all of the edges from least weight to greatest. We then look at each edge in turn and decide whether we can safely add the edge to the tree or not - we do not want to add the edge if it's going to create a cycle in the tree. Let's go through a quick example. Suppose we want to find a minimum spanning tree for the graph below.</p>

                <img src="{{ asset('images/blog/graph.png') }}" alt="Graph">

                <p>We start by ordering the edges from least to greatest weight: 1-3 (edge between 1 and 3), 2-3, 2-4, 3-4, 1-5, 3-5, 1-2. If two or more edges have the same weight, the order in which these edges are written down doesn't matter (we could've just as well have swapped the positions of 1-3 and 2-3, for example). The other big idea that I mentioned is that we need some way to check and see if the edge we're about to try adding will create a cycle. This will be done using an adjacency list as it will keep track of who's connected to who in terms of nodes. Now, let's begin to construct our new graph. We start with just the nodes, as shown below, and an adjacency list with all nodes' parents initialized to -1 (-1 will be the placeholder for a node whose parent is itself - ie it's not connected to any other node(s)):</p>

                <img src="{{ asset('images/blog/step_1.png') }}" alt="Graph">
                <img src="{{ asset('images/blog/adj_1.png') }}" alt="Adjacency list">

                <p>The first edge in our ordered list is 1-3. I'll save the explanantion as to how we check the adjacency list to determine if it's safe to add the edge for the next step. For now, just know that we can safely add this edge without creating a cycle. I'll update the adjacency list to show this new connection between nodes 1 and 3. The updated graph and adjacency list are shown below:</p>

                <img src="{{ asset('images/blog/step_2.png') }}" alt="Graph">
                <img src="{{ asset('images/blog/adj_2.png') }}" alt="Adjacency list">

                <p>I'm going to take this opportunity to explain the findHead(x) function in the code below as it will help us understand how the adjacency list works. For a given node x, the algorithm follows the chain of arrows in the adjacency list, starting at x, until the end of the 'arrow chain' is reached. We'll call the node at the end of the chain the 'head' of the chain and the function returns this value. For example, findHead(3) would return 1 for adjacency list in the image above. How does this help us to determine whether we can add an edge or not? Let's work with the next edge, 2-3, to help us understand. findHead(2) returns itself, namely 2, as there's no chain of arrows to follow and findHead(3) returns a head of 1. Because findHead() returns a different head for these two nodes, we know that these two nodes are not in any way already connected. We can safely add an edge connecting them without creating a cycle. This new connection must be reflected in the adjacency list so we set the value findHead(2), namely 2, as the parent of findHead(3), which is 1. This is reflected in the adjacency list below. I'll skip the explanation for the next edge 2-4. The updated graph and adjacency list is shown below right.</p>
                ...
                <p>Next up is edge 3-4. findHead(3) returns 2 and findHead(4) also returns 2. This tells us that they are already connected. We also know from our approach to the solution that there is no cycle between any of these nodes - in other words, they're connected with a minimum number of edges and adding another edge would create a cycle. Therefore, we must reject adding edge 3-4. We can see from the graph below that adding the edge would create a cycle between 2-3-4.</p>

                <img src="{{ asset('images/blog/step_5.png') }}" alt="Graph">

                <p>The next edge to try adding is 1-5. findHead(5) returns 5 and findHead(1) returns 2 so we know that they are not connected and we can safely add this edge. The tree is now a spanning tree of minimum weight so the algorithm is complete!</p>
